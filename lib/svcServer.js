...
var asyncMapJsonList = function *() {
    ...
    var mapDir = null;
    var pathPrefix = global.options.domain;
    ...
    filteredContList = filteredFileList.map(function (vFileName) {
        ...
        if(global.options.addr){
            ...
            Object.keys(vFileJSON.res).forEach(function (k) {
                ...
                if(v.type !== 'vm' && v.uri[0] === '/' && webRoot && v.uri.indexOf(webRoot) != 1){
                    v.uri = pathPrefix + '/' + webRoot + v.uri;
                }else if(v.type !== 'vm' && v.uri[0] === '/'){
                    v.uri = pathPrefix + v.uri;
                }
            });
