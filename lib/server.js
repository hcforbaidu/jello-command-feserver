...
app.use(function* log(next) {
    console.info('LOG: ', this.path, this.method, this.ip.replace('::ffff:', ''));

    if(/\.htm.vm$/.test(this.path)){
        this.path = this.path.replace('.htm.vm', '.vm');
    }

    if(/\.vm$/.test(this.path)){
        // CRM Path: /management/platform/project-common:platform/layout.vm
        var pathList = this.path.split(':');
        var prevPathList = pathList[0].split('/');
        var path = prevPathList[prevPathList.length - 1] + '/' + pathList[1];
        console.warn('LOG: Request Redirect: ' + this.path + ' --> ' + path);
        this.path = path;
    }
